...
import { isLogged, logOut } from '../hepers/auth'
              ...
              <Link className="navbar-brand" to="/">
               ...
               { jwt ?
                ...
                <ul style={ { width : '100%' , display : 'flex' , justifyContent : 'center' , position : 'absolute'  } } className="">
                  <div className='d-flex'>
                    <li className="nav-item">
                    <Link onClick={()=> dispatch({type:"HIDE_USER_ERROR"})} className="navlink mx-2" aria-current="page" to="/login">LogIn</Link>
                    </li>
                    <li className="nav-item mx-2">
                    <Link onClick={()=> dispatch({type:"HIDE_USER_ERROR"})} className="navlink" to="/register">Register</Link>
                    </li>
                  </div>
                </ul>
              }

              {
                ...
                <div style={{ justifyContent : 'space-between' , 
                 ...
                 { 
                    ...
                    {
                      jwt ? 
                      (<Link className='d-flex justify-content-center align-items-center'  to={jwt && jwt.user && `/user/${jwt.user && jwt.user._id}`} >
